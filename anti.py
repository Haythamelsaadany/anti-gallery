        ...
        if up:
            ...
            if st.button("🚀 ابدأ التحليل والبحث"):
                with st.spinner("جاري التحليل..."):
                    ...
                    # ترميز النص ليكون صالحاً للروابط
                    encoded_q = urllib.parse.quote_plus(clean_desc)
                    
                    st.divider()
                    st.subheader("🔗 روابط البحث المباشرة:")
                    
                    # الروابط بهيكلها الصحيح والكامل (تم تصحيحها لتعمل 100%)
                    ebay_url = f"https://www.ebay.com{encoded_q}"
                    google_url = f"https://www.google.com{encoded_q}&tbm=isch"
                    
                    col1, col2 = st.columns(2)
                    col1.link_button("🛒 شاهد الأسعار في eBay", ebay_url, use_container_width=True)
                    col2.link_button("🔍 البحث في Google Images", google_url, use_container_width=True)
